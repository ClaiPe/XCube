...
if __name__ == '__main__':
    ...
    # Allow runtime defaults from YAML while keeping CLI override priority.
    if program_args.gpus is None and hasattr(model_args, 'gpus'):
        program_args.gpus = model_args.gpus
    if program_args.max_epochs is None and hasattr(model_args, 'max_epochs'):
        program_args.max_epochs = model_args.max_epochs
    if program_args.logger_type == 'wandb' and hasattr(model_args, 'logger_type'):
        program_args.logger_type = model_args.logger_type
    if str(program_args.model_precision) == '32' and hasattr(model_args, 'model_precision'):
        program_args.model_precision = model_args.model_precision
