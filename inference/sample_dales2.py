...
from xcube.utils.vis_util import random_seed, dales2_palette, dales2_mapping
...
config_coarse = "configs/dales2/train_diffusion_dense.yaml"
ckpt_coarse = "checkpoints/dales2/diffusion_dense/version_0/checkpoints/last.ckpt"
    
config_fine = "configs/dales2/train_diffusion_sparse.yaml"
ckpt_fine = "checkpoints/dales2/diffusion_sparse/version_0/checkpoints/last.ckpt"
    ...
    while True:
        ...
        for batch_idx in range(output_x.grid.grid_count):
            ...
            fine_normal_feat = (
                ...
                else coarse_normal_feat
            )
            fine_semantic_feat = (
                res.semantic_features[-1].feature[batch_idx].jdata
                if res.semantic_features
                else coarse_semantic_feat
            )

            ## coarse stage
            result_dict = {}
            result_dict['coarse_xyz'] = output_x_coarse.grid.grid_to_world(output_x_coarse.grid[batch_idx].ijk.float()).jdata.cpu().numpy()
            result_dict['coarse_normal'] = coarse_normal_feat.cpu().numpy()
            result_dict['coarse_semantic'] = coarse_semantic_feat.cpu().numpy().argmax(axis=1)
            ## fine stage
            result_dict['fine_xyz'] = output_x.grid.grid_to_world(output_x.grid[batch_idx].ijk.float()).jdata.cpu().numpy()
            result_dict['fine_normal'] = fine_normal_feat.cpu().numpy()
            result_dict['fine_semantic'] = fine_semantic_feat.cpu().numpy().argmax(axis=1)
            # save result_dict
            ...
            if known_args.extract_mesh:
                semantics: np.ndarray = dales2_mapping[result_dict['fine_semantic']]
                sem_color = dales2_palette[semantics]

                pd_xyz = output_x.grid.grid_to_world(output_x.grid[batch_idx].ijk.float()).jdata
                normal = fine_normal_feat
